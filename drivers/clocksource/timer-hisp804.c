...
static int hisp804_clockevent_setup(struct hisp804_clockevent_device *hiclkevt)
{
    ...
    BUG_ON(request_irq(clkevt->irq, hisp804_clockevent_timer_interrupt,
                    IRQF_TIMER | IRQF_NOBALANCING,
                    hiclkevt->name, &hiclkevt->clkevt));
...
static int hisp804_clockevent_cpu_online(unsigned int cpu)
{
    hisp804_clockevent_setup(this_cpu_ptr(hisp804_clkevt));
    return 0;
}

static int hisp804_clockevent_cpu_dead(unsigned int cpu)
{
...
static int __init hisp804_timer_init(struct device_node *node)
{
    ...
    ret = cpuhp_setup_state(CPUHP_AP_ONLINE_DYN, "clockevents/hisp804:online",
                      hisp804_clockevent_cpu_online,
                      hisp804_clockevent_cpu_dead);
    if (ret) {
        goto out_notifier;
    }
