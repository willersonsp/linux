...
#ifdef CONFIG_IAS_SESSION
	if (p->session.session_prio)
		enqueue_session_thread(rq, p);
#endif
...
#ifdef CONFIG_IAS_SESSION
	if (p->session.session_prio) {
		int session_cpu = find_session_cpu(p);
...
#ifdef CONFIG_IAS_SESSION
	if (curr->session.session_prio)
		return;

	if (!curr->session.session_prio && p->session.session_prio)
		goto preempt;
#endif
...
again:
	...
	if (!list_empty(&rq->session.session_list)) {
		...
		if (p) {
			...
			trace_sched_session_pick_op(p, "pick_session");
			goto done;
		}
	}
#endif
...
#ifdef CONFIG_IAS_SESSION
	if (p->session.session_prio && tsk_cache_hot <= 0) {
		if (env->dst_rq->session.session_nr_running -
		    env->src_rq->session.session_nr_running >= 2)
			return 1;
		return 0;
	}
#endif
